...
namespace System
{
    ...
    public class Logger
    {
        ...
        public bool initialize(bool debug)
        {
            ...
            //String binDir = binFullPath.Substring(0, idx + 1)+"logs";


            string binDir = Utils.GetCurrentDirectory() + "logs\\";
            binDir.CreateDirectory();

            return initialize(debug, binDir + "{0}.log".SFormat(DateTime.Now.ToString("yyyyMMdd")), 1024000, 10);
        }
        ...
        public bool initialize(bool debug, String logName, int maxFileSize, int maxCount)
        {
            ...
            {
                Console.WriteLine("Unable to create log {0}.  The exception is {1}.", logName, e);
                m_logWriter = null;
            }
            return true;
        ...
        {
            if (m_logToConsole) Console.WriteLine(msg);
            ...
            if (m_logWriter != null)
            {
                lock (m_logWriter)
                {
                    ...
                    {
                        m_logWriter = null;
                        Console.WriteLine("Exception occurred when writing to log: {0}", e);
                    }
                }
            }
        }
