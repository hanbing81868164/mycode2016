...
namespace System
{
    ...
    //var bytes = FileHelper.GetBytes(filePath);
    //var sinodetector = new IdentifyEncoding();
    //var encoding = sinodetector.GetEncoding(sinodetector.GetEncodingName(new Uri(filePath)));

    //string fileData = bytes.ToString(encoding);
        ...
        internal static string[] nicename = new string[]
   {
    ...
    , "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER"
        ...
        public virtual string GetEncodingName(Uri testurl)
        {
            ...
            IO.Stream chinesestream;
            try
            {
                chinesestream = Net.WebRequest.Create(testurl.AbsoluteUri).GetResponse().GetResponseStream();
        ...
        /// <summary>
        /// 从指定的 <see cref="IO.FileInfo"/> 中判断编码类型
        /// </summary>
        /// <param name="testfile">要判断的 <see cref="IO.FileInfo"/> </param>
        /// <returns>返回编码类型("GB2312", "GBK", "HZ", "Big5", "CNS 11643", "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER")</returns>
        /// <example>
        /// 以下示例演示了如何调用 <see cref="GetEncodingName"/> 方法：
        /// <code>
        ...
        /// </code>
        /// </example>
        public virtual string GetEncodingName(IO.FileInfo testfile)
        {
            IO.FileStream chinesefile;
            sbyte[] rawtext;
            rawtext = new sbyte[(int)FileLength(testfile)];
            try
            {
                chinesefile = new IO.FileStream(testfile.FullName, System.IO.FileMode.Open, System.IO.FileAccess.Read);
        ...
        /// </summary>
        /// <param name="rawtext">要判断的 <see cref="IO.FileInfo"/> </param>
        /// <returns>返回编码类型("GB2312", "GBK", "HZ", "Big5", "CNS 11643", "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER")</returns>
        /// <example>
        /// 以下示例演示了如何调用 <see cref="GetEncodingName"/> 方法：
        /// <code>
        ...
        /// <exception cref="IO.IOException">发生 I/O 错误。</exception>
        /// <exception cref="NotSupportedException">流不支持读取。</exception>
        /// <exception cref="ObjectDisposedException">在流关闭后调用方法。</exception>
        public static int ReadInput(IO.Stream sourceStream, ref sbyte[] target, int start, int count)
        {
        ...
        /// <exception cref="NotSupportedException">流不支持读取。</exception>
        /// <exception cref="ObjectDisposedException">在流关闭后调用方法。</exception>
        public static int ReadInput(IO.TextReader sourceTextReader, ref sbyte[] target, int start, int count)
        {
            // Returns 0 bytes if not enough space in target
            if (target.Length == 0)
                return 0;
        ...
        public static long FileLength(IO.FileInfo file)
        {
