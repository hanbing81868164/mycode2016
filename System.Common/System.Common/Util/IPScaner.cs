...
{
    /// <summary> 
    /// 通过IP地址从纯真IP数据库里查询出IP所在的地区位置
    /// 使用示例:
    ///string IP = this.Context.Request.ServerVariables["REMOTE_ADDR"].ToString();//IP地址
    ///Forum.BLL.Other.IPScaner ip = new Forum.BLL.Other.IPScaner();//实例化类
    ///string IPForm = ip.IPLocation(Server.MapPath("QQWry.DAT"), IP).Replace("CZ88.NET", "");//返回所在地区
    /// </summary> 
    public class IPScaner
    {
        ...
        /// 返回实际来源
        /// </summary>
        /// <param name="dataPath">dat文件地址</param>
